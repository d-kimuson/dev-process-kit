...
<dpk-template-grill storage-key="order-cancel-review">…</dpk-template-grill>
```
...
Declare `capabilities: { comments: {}, db: {} }` on the first publish, and the review rail (in grill: the footer) offers **Claude に送る** in place of copying: the brief arrives in your session as a comment sent to Claude, pinned to the template, and the user no longer pastes anything.
...
After a brief, rewrite the same file and republish it to the same artifact (same file path in the session, or the artifact's URL) so the user keeps one link, and stale / no-op actions clear themselves against the new base. Omit `capabilities` on a republish: the declaration carries forward.
