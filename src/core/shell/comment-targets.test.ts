...
import { DpkTemplatePrototype } from '../../templates/prototype/element';
import '../../index';

const mount = async () => {
  const host = document.createElement('dpk-template-prototype');
  if (!(host instanceof DpkTemplatePrototype)) throw new Error('not upgraded');
  host.storage = 'memory';
...
describe('component targets in the common shell', () => {
  it('acknowledges only valid local submissions without opening the rail', async () => {
    ...
    expect(host.api.comments[0]?.payload).toEqual({ body: 'local note' });
    ...
    expect(host.api.actions).toHaveLength(1);
  });

  it('never passes a rejected nested submission to the outer template', async () => {
    ...
    expect(inner.api.actions).toEqual([]);
    expect(outer.api.actions).toEqual([]);
  });
  ...
  it('uses the standard template rail for a composed request from shadow content', async () => {
    ...
    host.api.comment('element:custom/shape/a', 'note');
    expect(host.api.comments).toHaveLength(1);
    expect(host.api.exportBrief()).toContain('Shape A');
    shape.remove();
    await Promise.resolve();
    await host.updateComplete;
  ...
  it('rejects malformed providers and requests and keeps nested template targets isolated', async () => {
    const outer = await mount();
    const inner = await mount();
    outer.append(inner);
    const shape = provider();
    ...
    expect(outer.notesOpen).toBe(false);
    outer.dispatchEvent(new CustomEvent('dpk-comment-request', { detail: { target: 123 } }));
    expect(outer.notesOpen).toBe(false);
    outer.api.comment('element:custom/shape/a', 'wrong owner');
    expect(outer.api.stale).toHaveLength(1);
    expect(outer.controller.definition.commentTargets(outer.api.state, {})).not.toContainEqual({
      value: 'step:hijack',
      label: 'Bad',
    });
  });
  ...
  it('records component element actions, hands them back to their owner and lists its results', async () => {
    ...
    const act = (detail: unknown): boolean =>
      shape.dispatchEvent(
        new CustomEvent('dpk-element-action', { detail, bubbles: true, composed: true, cancelable: true }),
      );
    ...
    expect(shape.elementActions.map((action) => action.id)).toEqual(host.api.actions.map((action) => action.id));
    ...
    expect(host.controller.definition.describe(shape.elementActions[0] as never, host.api.state).title).toBe('Grow');
    const count = received.length;
    host.api.comment('element:custom/shape/a', 'unrelated');
    await Promise.resolve();
    // Comments are not component actions: the list is not reassigned.
    expect(received).toHaveLength(count);
    host.controller.removeAction(host.api.actions[0]?.id ?? '');
