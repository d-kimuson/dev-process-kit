...
const controller = () => new ArtifactController({ definition: prototypeDefinition, base: base(), storage: null });
...
describe('qualified prototype references', () => {
  ...
  it('validates a batch against the compacted candidate, without partial commit', () => {
    const save = vi.fn();
    const c = new ArtifactController({
  ...
  it.each(['b.other.x', 'missing.other.x'])('rejects foreign or nonexistent move/reorder anchors: %s', (after) => {
    ...
    const c = new ArtifactController({ definition: prototypeDefinition, base, storage: null });
