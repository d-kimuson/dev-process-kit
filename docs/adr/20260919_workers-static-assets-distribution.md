# Publish with Workers Static Assets and Git-managed release artifacts

Distribute the single ESM bundle and user-facing documentation through Cloudflare Workers
Static Assets at `https://dev-process-kit.kimuson.dev`. Commit release artifacts to Git
and retain previous versions. For now, do not introduce Worker code or a separate artifact store
for distribution.
...
Generated single-file HTML artifacts load the framework from a version-pinned URL.
...
Committing build artifacts increases repository size and adds generated diffs. At this stage,
...
- Commit release artifacts under `public/dev-process-kit@<version>/` and deploy all of `public/`,
...
- Publishing a release adds a new version's artifacts. Deployments do not need to rebuild
  previous releases.
