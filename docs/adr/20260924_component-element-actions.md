# Record component edits as element actions in the enclosing template's draft

A component inside a template (a diagram, an author-owned provider) records change requests against its own elements in the enclosing template's draft, next to that template's actions and comments. The core stores and orders these element actions but never interprets them; the component replays them against its own data and reports how each one applied.
...
- **The enclosing template's draft, uninterpreted.** One draft, one review list and one brief, as for comments; the component remains the only code that knows what its actions mean.

## Decision

- **Element actions live in the enclosing template's draft.** A component emits cancelable, bubbling, composed `dpk-element-action` with `{ type, target, payload }`. `type` is `CONSTANT_CASE`, and `target` must be one of the component's registered comment targets. The nearest template validates and stores the action, then calls `preventDefault()` once it has accepted it, as with `dpk-comment-submit`.
...
- **The component replays and reports.** The template element assigns each provider its own actions (`elementActions`, keyed by the provider's unique `id`) synchronously after every change. The component applies them to its own authored data and exposes `elementActionResults`: `{ id, title, summary?, tone?, stale? }` per action. It then emits `dpk-comment-targets-change`. The template element uses these results for applied/stale, the review list and the brief. A result counts only when the provider that owns the action reports it, and an action with no result is stale (`target-missing`), exactly like a comment on a component that is not there.
...
- A standalone component (outside any template) has nobody to accept its actions. It must treat an unacknowledged event as a failure and keep the user's input.
