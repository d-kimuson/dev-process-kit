...
- **A template entry carries only what that template renders.** The core API, the review rail and inline editing (the artifact shell renders the rail for every template), and that one template. A template entry deliberately does _not_ re-export the components it does not ship: an entry that exports the diagram classes cannot tree-shake them away, which is the whole point of the split.
...
- A page that uses one template downloads about a third of what it used to. The union entry is unchanged, so existing artifacts and examples that pin `index.js` keep working, and older releases keep the single file they shipped.
...
- Every entry registers the same core elements, so the rail is available whichever entry a page loads; `window.artifactFramework.templates` now lists what the loaded entry actually registered (it used to claim `['prototype']` regardless).
